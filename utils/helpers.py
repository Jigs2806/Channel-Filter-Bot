...
async def get_groups():
    ...
    cursor = grp_col.find({})
    group_list = await cursor.to_list(length=int(count))
    return count, group_list
...
async def get_users():
    ...
    user_list = await cursor.to_list(length=int(count))
    return count, user_list

async def save_dlt_message(message, time):
    data = {"chat_id": message.chat.id, "message_id": message.id, "time": time}
...
async def force_sub(bot, message):
    ...
    try:
        ...
        await message.reply(f"⚠ Dear User {message.from_user.mention}!\n\nto send a message in the group, you have to join our channel to message here",
